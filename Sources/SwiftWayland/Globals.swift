...
//             fatalError(
//                 "Maximum version (\(version.upperBound)) of \(interface.name) was higher than the proxy's maximum version (\(interface.interfaceVersion)); outdated wayland XML files"
//             )
//         }

//         // mutex???
//         guard let global = self.globals.first(where: { interface.name == $0.interfaceName }) else {
//             throw .notPresent
//         }
...
//         let version = min(version.upperBound, UInt32(interface.interfaceVersion))
