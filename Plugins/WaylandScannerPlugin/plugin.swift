...
struct WaylandScannerPlugin: BuildToolPlugin {
    func createBuildCommands(context: PluginContext, target: Target) async throws -> [Command] {
        let tool = try context.tool(named: "WaylandScannerCLI")
        let pkg = context.package.directoryURL
        let workDir = context.pluginWorkDirectoryURL

        switch target.name {
        case "WaylandClient":
            return [coreWaylandCommand(tool: tool, pkg: pkg, workDir: workDir)]
        case "WaylandClientProtocols":
            return protocolCommands(tool: tool, pkg: pkg, workDir: workDir)
        default:
            return []
        }
    }

    private func coreWaylandCommand(tool: PluginContext.Tool, pkg: URL, workDir: URL) -> Command {
        let input = pkg.appending(path: "wayland.xml")
        let output = workDir.appending(path: "Wayland.swift")
        return .buildCommand(
            displayName: "WaylandScanner: wayland core protocols",
            executable: tool.url,
            arguments: [
                "client",
                "--input-files", input.path(percentEncoded: false),
                "--no-import",
                "--output-file", output.path(percentEncoded: false),
            ],
            inputFiles: [input],
            outputFiles: [output]
        )
    ...
    private func protocolCommands(tool: PluginContext.Tool, pkg: URL, workDir: URL) -> [Command] {
        func ins(_ paths: [String]) -> [URL] { paths.map { pkg.appending(path: $0) } }

        func cmd(
            _ name: String, output: String, inputs: [String],
            trait: String, prefixMaps: [String: String] = [:]
        ) -> Command {
            let inputURLs = ins(inputs)
            let outputURL = workDir.appending(path: output)
            var args =
                ["client", "--input-files"] + inputURLs.map { $0.path(percentEncoded: false) } + [
        ...
        return [
            cmd("XDG", output: "xdg.swift", inputs: xdgProtocols, trait: "XDG"),
            cmd("XWayland", output: "xwayland.swift", inputs: xwaylandProtocols, trait: "XWAYLAND"),
